...
function installSshpass {
	...
	if resourceExists $SSHPASS_FILE; then
		installSshpassLocally
	else
		installSshpassRemotely
	fi
}

function installSshpassRemotely {
	echo "install sshpass from remote"
	yum -y install wget
...
function installSshpassLocally {
	echo "start to install local sshpass"
	whoami
	tar -zxf $VAG_RESOURCE/$SSHPASS_FILE -C /tmp
	echo "start to build with user `whoami`"
	cd /tmp/sshpass-1.06 && ./configure --prefix=/opt/sshpass \
	&& make && make install
